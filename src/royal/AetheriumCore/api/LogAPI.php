...
class LogAPI{
    public static function InsertLogBlockPlaced(Block $block, Player $player)
    {
        ...
        $position = $x.":".$y.":".$z;
        Variables::$logPlayerBlockPlaced[$position]= array_merge(Variables::$logPlayerBlockPlaced[$position]["pseudo"] = $player->getName(),Variables::$logPlayerBlockPlaced[$position]["date"] = Date("Y-m-d H:i:s"), ["position" => $position]);
    }
    ...
    public static function getLogBlockPlaced(Block $block, Player $player){
        ...
        $player->sendMessage("Le block a la position : ". $position." a été placé par " . `$pseudo`);
    }
